...
class ABB<E extends Comparable<E>> {
    ...
    public Lista<E> recortar(E inicio, E fim) {
        Lista<E> lista = new Lista<>();
        recortar(this.raiz, inicio, fim, lista);
        return lista;
    }

    private void recortar(No<E> raizArvore, E inicio, E fim, Lista<E> lista) {
        if (raizArvore != null) {
            recortar(raizArvore.getEsquerda(), inicio, fim, lista);
            if (raizArvore.getItem().compareTo(inicio) > 0 && raizArvore.getItem().compareTo(fim) < 0) {
                lista.inserirFinal(raizArvore.getItem());
            }
...
public class App {
    public static void main(String[] args) throws Exception {
        ...
        while (!entrada.equals("FIM")) {
            ...
            Lista<Medalhista> lista = arvoreMedalhista.recortar(medalhistas.get(entradaSeparada[0]),
                    medalhistas.get(entradaSeparada[1]));
            System.out.println(entrada);
            Celula<Medalhista> atual = lista.getPrimeiro().getProximo();
            while (atual != null) {
                
                if (entradaSeparada[2].equals("OURO") && atual.getItem().getGoldMedal() > 0) {
                    System.out.println(atual.getItem());
                    System.out.println("Quantidade de medalhas de OURO: " + atual.getItem().getGoldMedal()  + "\n");

                } else if (entradaSeparada[2].equals("PRATA") && atual.getItem().getSilverMedal() > 0) {
                    System.out.println(atual.getItem());
                    System.out.println("Quantidade de medalhas de PRATA: " + atual.getItem().getSilverMedal() + "\n");

                } else if (entradaSeparada[2].equals("BRONZE") && atual.getItem().getBronzeMedal() > 0) {
                    System.out.println(atual.getItem());
                    System.out.println("Quantidade de medalhas de BRONZE: " + atual.getItem().getBronzeMedal() + "\n");
                }
                atual = atual.getProximo();
            }
            entrada = sc.nextLine();
